...
<%@ include file="../admin/multipleCondition.jsp" %>
<h4>가게 목록</h4>
<c:if test="${count > 0}">
...
<!-- << 처음으로 < 이전으로 -->  
   <c:if test="${numPageGroup > 1}">
        <a href="./restaurant/shopList?pageNum=1">[<<]</a>
        <a href="./restaurant/shopList?pageNum=${(numPageGroup-2)*pageGroupSize+1 }">[<]</a>
   </c:if> 
   
  <c:forEach var="i" begin="${startPage}" end="${endPage}">
       <a href="/restaurant/shopList?pageNum=${i}">[       
...
<!-- >> 맨뒤로 > 다음으로 -->
   <c:if test="${numPageGroup < pageGroupCount}">
      <a href="./restaurant/shopList?pageNum=${numPageGroup*pageGroupSize+1}">[>]</a>
        
      <a href="./restaurant/shopList?pageNum=${maxPage}">[>>]</a>
