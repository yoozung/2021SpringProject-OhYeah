...
@Component("needToConsumerInterceptor")
@Slf4j
public class NeedToConsumerInterceptor implements HandlerInterceptor {

	@Override
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		log.debug(">>>>> NeedToConsumerInterceptor");
		boolean isLogin = (boolean) request.getAttribute("isLogin");
		boolean isConsumer = (boolean) request.getAttribute("isConsumer");
		...
		if(!isLogin) {
			...
			out.append("location.replace('/signInPage');");
			out.append("</script>");
			return false;
		}
